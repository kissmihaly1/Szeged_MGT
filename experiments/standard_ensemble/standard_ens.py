...
dev_df['encoded_model'] = dev_df['model'].apply(lambda x: label_encoder.transform([x])[0] if x in label_encoder.classes_ else print("asd"))
        ...
        for step, batch in enumerate(tqdm(train_loader, desc=f'Training Epoch {epoch + 1}/{epochs}')):
            ...
            if (step + 1) % val_steps == 0:
                ...
                if best_f1 > best_f1_overall:
                    ...
                    best_model_path = f"Model_{model_index}_val_{val_count}_gpu1_val{f1}"
...
best_model = DebertaForSequenceClassification.from_pretrained("microsoft/deberta-base", num_labels=41)
...
with torch.no_grad():
    for batch_idx, batch in enumerate(tqdm(dev_loader, desc='Testing on dev set')):
        ...
        for i, prob in enumerate(probabilities.cpu().numpy()):
                    dev_df.at[batch_idx * dev_loader.batch_size + i, 'softmax_logits'] = prob.tolist()

...
for model_index in range(1):
    ...
    if model_index == 0:
        ...
        set_seed(seed)
        if log_wandb:
            wandb.init(project="-", name=f"deberta_run_{model_index + 1}_gpu3_5group_seed41", reinit=True)
    elif model_index == 1:
        seed = 29
        set_seed(seed)
        if log_wandb:
            wandb.init(project="-", name=f"deberta_run_{model_index + 1}_gpu3_5group_seed29", reinit=True)

    elif model_index == 2:
        seed = 67
        set_seed(seed)
        if log_wandb:
            wandb.init(project="-", name=f"deberta_run_{model_index + 1}_gpu3_5group_seed67", reinit=True)
        ...
        for step, batch in enumerate(tqdm(train_loader, desc=f'Training Epoch {epoch + 1}/{epochs}')):
            ...
            if (step + 1) % val_steps == 0:
                ...
                if best_f1 > best_f1_overall:
                    ...
                    best_model_path = f"Model_5groups_seed{seed}"
    ...
    best_model = DebertaForSequenceClassification.from_pretrained("microsoft/deberta-base", num_labels=5)

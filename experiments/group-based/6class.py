...
model_to_group.update({model: 1 for model in group_1})
model_to_group.update({model: 2 for model in group_2})
model_to_group.update({model: 3 for model in group_3})
model_to_group.update({model: 4 for model in group_4})
model_to_group.update({model: 5 for model in group_5})
...
for model_index in range(3):
    ...
    if model_index == 0:
        ...
        set_seed(seed)
        if log_wandb:
            wandb.init(project="-", name=f"deberta_run_{model_index + 1}_gpu2_6group_seed4", reinit=True)
    elif model_index == 1:
        seed = 15
        set_seed(seed)
        if log_wandb:
            wandb.init(project="-", name=f"deberta_run_{model_index + 1}_gpu2_6group_seed15", reinit=True)

    elif model_index == 2:
        seed = 42
        set_seed(seed)
        if log_wandb:
            wandb.init(project="-", name=f"deberta_run_{model_index + 1}_gpu2_6group_seed42", reinit=True)
        ...
        for step, batch in enumerate(tqdm(train_loader, desc=f'Training Epoch {epoch + 1}/{epochs}')):
            ...
            if (step + 1) % val_steps == 0:
                ...
                if best_f1 > best_f1_overall:
                    ...
                    best_model_path = f"Model_groups_seed{seed}"
    ...
    best_model = DebertaForSequenceClassification.from_pretrained("microsoft/deberta-base", num_labels=6)
